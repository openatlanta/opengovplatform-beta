...
integrate with 21 social networks and service providers, including
...
* DATA MAPPING:  With permission of the user, you can map third-party
  user profile data to specific Drupal Profile fields.
...
2. Visit admin/build/modules and enable the "Janrain Engage Core,"
   "Janrain Engage UI," and "Janrain Engage Widgets" modules. The
   "Janrain Engage Rules integration" module is optional if you would
   like those features. All of these modules can be found within the
   "Janrain Engage" fieldset.

3. Visit admin/user/permissions and configure available module
   permissions. This includes the "administer Janrain Engage"
   permission and the "manage own identities" permission.

4. If you want users to be able to create their own accounts using
   Janrain Engage, you should visit admin/user/settings and choose
   "Visitors can create accounts and no administrator approval is
   required" under the "Public registrations" setting.

5. Visit admin/user/rpx and enter your Janrain Engage API
...
6. Also at admin/user/rpx, configure other module settings
   related to the user interface, authentication, social sharing, and
   verification e-mails.

7. Visit admin/user/rpx/mapping and configure Field Mapping if you
   would like third-party profile data to be mapped to Drupal
   Profile fields.
...
* Token (http://drupal.org/project/token): Tokens are small bits of
  ...
  placeholders. By enabling this module, you will be able to use the
  functionality and see a browsable interface of available tokens in
  several administrative areas of the Janrain Engage module (where
  tokens are supported). You will be able to include a token (for
  instance, when defining your social sharing message text) simply by
  clicking on it).
