<?php
	...
	if($view->name == 'agency_wise_report') {
		$footer='';
		$footer.='<tr><td class="ds-list-head-new  ds-list-head-btm-left" style="padding-left:20px;">Total</td>'; 
		...
		$cat=array('catalog_type_raw_data','catalog_type_data_apps');
		for($i=0;$i<2;$i++) {
			$footer.='<td class="ds-list-item-new-summary" >';
			$result=db_query("SELECT count(distinct ds.nid) as cnt 
			    FROM content_type_dataset ds 
			    INNER JOIN workflow_node WF On WF.nid=ds.nid 
			    WHERE ds.field_ds_catlog_type_type ='$cat[$i]' AND WF.sid=10");
			$val=db_query("SELECT count(distinct ds.nid) as cnt  
			    FROM  content_type_dataset ds 
			    INNER JOIN content_type_policy_program_open_government ppog ON ds.nid=ppog.nid 
			    INNER JOIN workflow_node WF On WF.nid=ds.nid  
			    where ( ds.field_ds_catlog_type_type ='$cat[$i]') 
			    AND ppog.field_ppog_high_value_dataset_value='Yes' AND WF.sid=10 ");
		...
		$result=db_query("SELECT count(distinct ds.nid) as cnt 
		    FROM content_type_dataset ds 
		    INNER JOIN workflow_node WF On WF.nid=ds.nid 
		    where ds.field_ds_catlog_type_type IS NOT NULL AND WF.sid=10");
		$val=db_query("SELECT count(distinct ds.nid) as cnt 
		    FROM  content_type_dataset ds 
		    INNER JOIN content_type_policy_program_open_government ppog ON ds.nid=ppog.nid 
		    INNER JOIN workflow_node WF On WF.nid=ds.nid 
		    where ppog.field_ppog_high_value_dataset_value='Yes' AND ds.field_ds_catlog_type_type IS NOT NULL AND WF.sid=10");
		if($row=mysql_fetch_object($result))
			$total=$row->cnt;
		if($high=mysql_fetch_object($val))
			$val=$high->cnt;
