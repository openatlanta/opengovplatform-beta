  ...
  if($catalog_type = get_filter('catalog_type')) {
    switch($catalog_type){
      case 'catalog_type_raw_data':
        ?>
        jQuery('.menu-10204 a').addClass('active');
        <?php 
        break;
      case 'catalog_type_data_apps':
        ?>
        jQuery('.menu-10178 a').addClass('active');
        <?php
        break;
    }
  }
...
<div class="<?php print $classes; ?>">
  ...
  <?php 
    ...
    ?>
    <div class="heading"> <?php print $search_title; ?>
      <div  style="display:none;" class="sort-select-box switch-js-enabled "><?php 
    	  print drupal_render($form['page-results']); 
    	?></div>
    	<?php 
    	$count=10;
    	$count=$_GET['results'];
      ...
      $page_uri=str_replace("&","&amp;",$page_uri);
    	?>
    	<div  class="sort-select-box cBoth switch-js-disabled"> Results Per Page: <?php 
    	if($count=='10')
    	print ' 10 |';
    	else
  ...
  <?php 
  if ($result){
    $catalog_type = 'Dataset';
  
    if(function_exists('get_filter')){
      $_catalog_type = get_filter('catalog_type');
    
      switch($_catalog_type){
        case 'catalog_type_raw_data':
          $catalog_type = 'Dataset';
          break;
        case 'catalog_type_data_apps':
      ...
      <div class="suggest-label">
        <div class="midTxt">
          <div class="midTxt-content">
            <a title="Suggest to the Department" href="<?php echo $base_url?>/suggest_dataset/?filter=catalog_type%3A<?php echo $_catalog_type;?>" >
              Could not find required <?php echo $catalog_type; ?>? Suggest to the Department
            </a> 
          </div>
          <?php /* ?>
          <div class="midTxt-content-image">
            <a title="Suggest Dataset" href="<?php echo $base_url?>/suggest_dataset/?filter=catalog_type%3A<?php echo $_catalog_type;?>" >Suggest</a>
          </div> 
          <?php */ ?>
        </div>
      </div>
  ...
  ?>
   <div class="box"><div class="content"><ul>
