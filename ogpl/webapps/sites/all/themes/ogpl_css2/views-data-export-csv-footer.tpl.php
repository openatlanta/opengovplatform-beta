...
if($view->name == 'agency_wise_report') {
	...
	for($i=0;$i<2;$i++) {
		...
		$val=db_query("SELECT count(distinct ds.nid) as cnt  
		    ...
		    INNER JOIN workflow_node WF On WF.nid=ds.nid  
		    where ( ds.field_ds_catlog_type_type ='$cat[$i]')  AND 
		    ppog.field_ppog_high_value_dataset_value='Yes' AND WF.sid=10 ");
	...
	$val=db_query("SELECT count(distinct ds.nid) as cnt  
	    ...
	    INNER JOIN content_type_policy_program_open_government ppog ON ds.nid=ppog.nid  
	    INNER JOIN workflow_node WF On WF.nid=ds.nid 
	    where ppog.field_ppog_high_value_dataset_value='Yes' 
	    AND ds.field_ds_catlog_type_type IS NOT NULL AND WF.sid=10");
