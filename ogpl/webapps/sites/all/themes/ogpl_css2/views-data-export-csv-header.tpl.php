...
if($view->name == 'agency_datasets' || $view->name == 'datasets_by_agency_per_month' || $view->name == 'datasets_per_month_and_year') {
	...
	$name = $node->title;
	$data = array('Ministry/Department', $agency);
}
...
if($view->name == 'agency_datasets') {
	...
	switch($catalog_type) {
		case '1': case 'catalog_type_raw_data': $catalog = array('Catalog Type', 'Dataset Catalogs'); break;
		case '2': case 'catalog_type_document': $catalog = array('Catalog Type', 'Document Catalogs'); break;
		case '3': case 'catalog_type_data_apps': $catalog = array('Catalog Type', 'App Catalogs'); break;
		case '4': case 'catalog_type_data_tools': $catalog = array('Catalog Type', 'Tool Catalogs'); break;
		case '5': case 'catalog_type_data_service': $catalog = array('Catalog Type', 'Service Catalogs'); break;
	}
	...
	switch($view->args[3]) {
		case 1: $arg0 = $view->args[0];
				...
				if($raw && $default!=0 ) { $catalog="Dataset Catalogs"; $default=0; }
