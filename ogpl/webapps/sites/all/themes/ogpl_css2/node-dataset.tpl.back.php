  ...
  <div class="cBoth"></div>
    ...
    <?php
      ...
      /* echo '<pre>' . print_r($node->field_dsat_downloadable, true) . '</pre>'; */
		  /*
		  $str2=explode('.', $node->field_dsat_downloadable[0]['value']['field_dam_datafile'][0]['filepath']);
		  $len2=count($str2);
		  $ext=$str2[($len2-1)];
		  
		  switch ($ext) {
		    case 'xml':
		      $fileicon = dms_customizations_get_file_icon('text/xml', false);
		      break;
		    default:
		      $fileicon = dms_customizations_get_file_icon($node->field_dsat_downloadable[0]['value']['field_dam_datafile'][0]['filemime'], false);
		  }
		  */
		  $fileicon = dms_customizations_get_file_icon($node->field_dsat_downloadable[0]['value']['field_dam_datafile'][0]['filemime'], false);
      
      $img = '<a target="_blank" href="'.$base_url.'/access-point-download-count?url='.$base_url.'/'.$node->field_dsat_downloadable[0]['value']['field_dam_datafile'][0]['filepath'].'&nid='.$node->nid.'">
      <img alt="'.$node->field_dsat_downloadable[0]['value']['field_dam_datafile'][0]['filename'].'" src="'.$base_url.'/'.drupal_get_path('theme', 'ogpl_css2').'/images/'.$fileicon.'"/></a>';
      ...
      $search=get_string_between($content,'<div class="field-label">Datafile:&nbsp;</div>','</div>');
     
      $search='<div class="field-label">Datafile:&nbsp;</div>'.$search;
      $replace='<div class="field-label">Datafile:&nbsp;</div><div class="field-items"><div class="field-item odd"><div class="filefield-file">'.$img. $download_count;
      
      $content=str_replace($search,$replace,$content);
    } else {
      /* echo '<pre>' . print_r($node->field_dsat_access_point, true) . '</pre>'; */
      /*
      $str2=explode('.', $node->field_dsat_access_point[0]['value']['field_elam_access_point'][0]['url']);
      $len2=count($str2);
      $ext=$str2[($len2-1)];
      ...
      switch ($ext) {
        case 'xml':
          ...
          break;
        default:
          $fileicon = dms_customizations_get_file_icon($node->field_dsat_access_point[0]['value']['field_elam_file_format'][0]['value']);
      }
      */
      $fileicon = dms_customizations_get_file_icon($node->field_dsat_access_point[0]['value']['field_elam_file_format'][0]['value']);
      
      $access_type_url=$node->field_dsat_access_point[0]['value']['field_elam_access_point'][0]['url'];
      
      $img = '<a target="_blank" href="'.$base_url.'/access-point-download-count?url='.$access_type_url.'&nid='.$node->nid.'">
      <img alt="'.$node->field_ds_access_method[0]['value']['field_atd_file_format'][0]['value'].'" src="'.$base_url.'/'.drupal_get_path('theme', 'ogpl_css2').'/images/'.$fileicon.'"/></a>';
      $download_count=db_result(db_query("Select download_count from web_download_count where nid=$node->nid"));
    ...
    /*
     *  Repalce all fieldset to div
     */
    
    $search   = '</legend>';
    $replace  = '</h2><div class="content">';
    $content  = str_replace($search,$replace,$content);
	...
	<div id="tabs-block" class="js-disable-hide">
		<ul class="list">
			<li class="contactOwner first active" title="Contact Dataset Owner">Contact Owner</li>
			<li class="ratings" title="Rate this dataset">Ratings</li>
			<li class="embed" title="Embed this dataset">Embed</li>		
            <li class="discuss last" title="Discuss this dataset">Discuss</li>				
		</ul>	
	</div>
    <?php } ?>
	</div>
