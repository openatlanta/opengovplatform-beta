...
<script type="text/javascript">
  $(function() {
    ...
    $('.views-exposed-form input[id^=edit-created-min], .views-exposed-form input[id^=edit-created-max]').each(function(){
    	vrm_cal_temp = new Date();
    	vrm_cal_inc = vrm_cal_temp.getTime();
    	if(vrm_cal_prev == vrm_cal_inc) {
    		i++;
    		vrm_date_name = vrm_cal_inc + '_' + i;
    	} else {
    		i = 0;
    		vrm_date_name = vrm_cal_inc;
    	}
        
  		$(this).attr('id', 'vrm_calander_'+vrm_date_name); 
  		
  		if (typeof console != 'undefined') {
  			console.log('#vrm_calander_' + vrm_date_name);
  		}
  		
  		$('#vrm_calander_' + vrm_date_name).datepicker({ dateFormat: 'dd-mm-yy' });
  		
  		vrm_cal_prev = vrm_cal_inc;
