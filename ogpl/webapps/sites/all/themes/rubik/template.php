...
function rubik_theme() {
  ...
  $items['dataset_node_form'] = array(
    'arguments' => array('form' => array()),
    'path' => drupal_get_path('theme', 'rubik') .'/templates',
    'template' => 'form-dataset',
    'preprocess functions' => array(
      'rubik_preprocess_form_filter',
      'rubik_preprocess_form_buttons',
...
function rubik_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
	global $user;
	...
	if(drupal_get_path_alias($_GET['q'])=='admin/build/block/list/rubik') {
		unset($breadcrumb['1']);
		unset($breadcrumb['3']);
		unset($breadcrumb['4']);
		$breadcrumb['2']=l('CMS/VRM/rubik Theme Blocks','admin/build/block/list/rubik');
	}
...
function rubik_removetab($label, &$vars) {
  $tabs = explode("\n", $vars['tabs']);
  $vars['tabs'] = '';
...
function rubik_form_element($element, $value) {
