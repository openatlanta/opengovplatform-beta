  ...
  <thead>
    <tr>
      <?php foreach ($header as $field => $label): ?>
		  ...
		  if($field=='title')
		  {
		   print '<th class="views-field views-field-<?php print $fields[$field]; ?> ds-list-head-new-first" style="width:210px; ">';
		   print $label; 
		   }
		  ...
		  {  print '<th class="views-field views-field-<?php print $fields[$field]; ?> ds-list-head-new-last" style="text-align:center; width:85px;">';
		    print $label; }
    ...
    <?php foreach ($rows as $count => $row): ?>
      <tr class="<?php print implode(' ', $row_classes[$count]); ?> ds-list-item-new">
        <?php foreach ($row as $field => $content): ?>
            ...
            <?php 
			if($field!='title')
				print ' <td class="views-field views-field-<?php print $fields[$field]; ?> " style="text-align:center;">';	
			else 
			  print  '<td class="views-field views-field-<?php print $fields[$field]; ?> " style="font-size:1em !important;">';
			print $content; 
...
<table class="header-width">
 <?php foreach ($header as $field => $label): ?>
	  ...
	  endforeach; ?>
	<tr><td class="ds-list-item-new" style="padding-left:10px">Total</td> 
	<?php
			...
			for($i=0;$i<5;$i++){
			print '<td class="ds-list-item-new-center" >';
			...
			$date=$row->date;
			print '<td class="ds-list-item-new-center">'.date('d/m/Y',$date).'</td>';
