...
function ogpl_css3_preprocess_search_theme_form(&$vars,$hook)
{
...
$vars['search']['search_theme_form'] = drupal_render($vars['form']['search_theme_form']);
  ...
  $vars['form']['submit']['#src'] = path_to_theme() . '/images/button.png';
...
function ogpl_css3_search_form($form)
{
  print_r($form);

}
function ogpl_css3_menu_item_link($link) {
 if ($link['title'] == 'Add To Favorites'){
    return '<a href="javascript:void(0);" onclick="add_to_favourites();" title="Add to Favorites">'.$link['title'].'</a>';
}
 return theme_menu_item_link($link);

}

function ogpl_css3_apachesolr_facet_link($facet_text, $path, $options = array(), $count, $active = FALSE, $num_found = NULL) {
  ...
  $options['attributes']['title']=$facet_text.' ('.$count.')';
...
function ogpl_css3_form_element($element, $value) {
...
function ogpl_css3_grid_block($element, $name) {
  $output = '';
  if ($element) {
 ...
 */
function ogpl_css3_grid_row($element, $name, $class='', $width='', $extra='') {
  $output = '';
  $extra = ($extra) ? ' ' . $extra : '';
  if ($element) {
    if ($class == 'full-width') {
...
function ogpl_css3_filefield_icon($file) {
    global $base_url;
    if (is_object($file)) {
        $file = (array) $file;
    }
