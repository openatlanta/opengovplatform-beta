...
var slideTimeIn;
/*
 * function startscroller(){ var sObj = $("#scroll-content"); var inContHeight =
 * $("#scroll-content").find("ul:first").height(); if(inContHeight >
 * $(sObj).parent().height()){ if(Math.abs(_GLOBAL_CNT) >=
 * parseInt($(sObj).height())){ _GLOBAL_CNT = 0; } _GLOBAL_CNT-= 1;
 * $(sObj).css("marginTop",_GLOBAL_CNT +"px"); $("#play").show();
 * $("#stop").show(); }else{ $(sObj).css("marginTop","0px"); $("#play").hide();
 * $("#stop").hide(); _GLOBAL_CNT = 0; } slideTimeIn =
 * setTimeout("startscroller()",40); }
 */
		...
		.ready(
				function() {
					...
					if ($("#web-contact-owner-form").length > 0) {
						$('#web-contact-owner-form select')
								.each(
										function(index) {
											$(this).addClass('purpose-field');
											$(this)
													.wrap(
															'<div id="contactowner-id" />');
										});
					}
					...
					if ($('.imageflow-visible').length > 0) {
						$('.imageflow-visible')
								.removeClass('imageflow-visible');
					}
					$("#context-block-region-text_resize").hide();
					if ($('#web-contact-owner-form').length > 0) {
						var datasetTitle = $('input[name="dataset-title"]')
								.val();
						$('select[name="purpose"]')
								.change(
										function() {
											...
											case 'Copyright Violation':
												$('input[name="subject"]')
														.val(
																'Your dataset "'
																		+ datasetTitle
																		+ '" has been flagged for copyright violation');
												break;
											case 'Offensive Content':
												$('input[name="subject"]')
														.val(
																'Your dataset "'
																		+ datasetTitle
																		+ '" has been flagged for offensive content');
												break;
											case 'Spam or Junk':
												$('input[name="subject"]')
														.val(
											...
											case 'Personal Information':
												$('input[name="subject"]')
														.val(
																'Your dataset "'
																		+ datasetTitle
																		+ '" has been flagged for containing personal information');
												break;
											case 'Other':
												$('input[name="subject"]').val(
														'data.gov.in: Feedback on "'
																+ datasetTitle
																+ '" dataset');
												break;
											}
					...
					if ($('#views-exposed-form-Catalogs-Search-page-1').length > 0) {
						if ($(".containers h1:first").html() != "Search") {
							var _fielVal = "Search for "
									+ $(".containers h1:first").html();
					...
					if ($(".metrics-menu").length > 0) {
						...
						var cmpstring = turl.substring(turl.indexOf('/'), turl
								.lastIndexOf('/'));
						cmpstring = cmpstring.substring(turl.indexOf('/'),
								cmpstring.lastIndexOf('/'));

						$(".metrics-menu .menuparent a")
								.each(
										function() {
											var activemode = ($(this).attr(
													'href').indexOf(
													'top10datasetreport') > -1 && turl
													.indexOf('top10datasetreport') > -1);
											if (cmpstring == $(this).attr(
													'href')
													|| activemode) {
												$(this).addClass('active');
												$(this).parents(".menuparent")
														.addClass(
																'active-trail');
											}
										});
						...
						if ($(".metrics-menu .menuparent a").hasClass("active")) {
							$(".metrics-menu .menuparent a").parent(
									".active-trail").find("a").filter(":first")
									.addClass("active");
						}
					}
					...
					if ($('#mainContent .content .dataset fieldset.group-ds-upload').length > 0) {
						$(
								'#mainContent .content .dataset fieldset.group-ds-upload')
								.find('legend').html('Download');
					}
					...
					if ($('.embed-block').length > 0) {
						...
						text_area_content = text_area_content.replace(':title',
								title);
						text_area_content = text_area_content.replace(
								':page_url', page_url);
						$('.embed-block textarea#embed_code').val(
								text_area_content);
						$('.econf-block #small').addClass(
								'econf-block-selected-color');
					}
					$(".iframe-dimensions").click(
							function() {
								$(".iframe-dimensions").removeClass(
										"econf-block-selected-color");
								$(this).addClass("econf-block-selected-color");
								var element_id = $(this).attr('id');
								var embed_html = $(
										'.embed-block textarea#embed_code')
										.val();
								var width;
								var height;
								var new_width = 500;
								...
								if (offset_width > offset_height) {
									is_width = false;
									html_1st_part = embed_html.substring(0,
											offset_width);
									html_2nd_part = embed_html.substring(
											offset_width, string_length);
								} else {
									html_1st_part = embed_html.substring(0,
											offset_height);
									html_2nd_part = embed_html.substring(
											offset_height, string_length);
								}
								width = $(
										$('.embed-block textarea#embed_code')
								...
								case 'large':
									...
									new_height = 808;
									if (is_width) {
										html_1st_part = html_1st_part.replace(
												width, new_width + 'px');
										html_2nd_part = html_2nd_part.replace(
								...
								$('.embed-block input#preview').removeClass(
										'disabled');
								$('.embed-block textarea#embed_code')
										.removeAttr('disabled');
								$('.embed-block input#preview').removeAttr(
										'disabled');
							});
					$(".custom-size")
							.keyup(
									function() {
										...
										if (offset_width > offset_height) {
											is_width = false;
											html_1st_part = embed_html
													.substring(0, offset_width);
											html_2nd_part = embed_html
													.substring(offset_width,
															string_length);
										} else {
											html_1st_part = embed_html
													.substring(0, offset_height);
											html_2nd_part = embed_html
													.substring(offset_height,
															string_length);
										}
										switch (element_id) {
										case 'ewidth':
											...
											if (is_width) {
												html_1st_part = html_1st_part
														.replace(width, value);
											} else {
												html_2nd_part = html_2nd_part
														.replace(width, value);
											}
											embed_html = html_1st_part
													+ html_2nd_part;
											$(
													'.embed-block textarea#embed_code')
													.val(embed_html);
					...
					if ($('.embed-code').length > 0) {
						var container_width = $('.embed-code .containers')
								.width();
						var attr_col = (container_width / 10).toFixed(0);
						var field_item_wt = (container_width * (45 / 100))
								.toFixed(0);
						$(".embed-code .containers textarea").each(
								function(index) {
									$(this).attr('cols', attr_col);
								});
						$(".embed-code .dataset .field-items").each(
								function(index) {
									$(this).attr('style', 'width:69%');
								});
						var page_url_woparam = $(location).attr('href');
						page_url_woparam = page_url_woparam.substring(0,
								page_url_woparam.indexOf('?'));
						var suggest_dataset_url = Drupal.settings.basePath
								+ 'suggest_dataset';
						$(".embed-code .embed-feature-links .embeded-link")
								.each(
										function(index) {
											if ($(this).hasClass(
...
function textCounter(max, textarename, desc) {
	if (document.getElementById(textarename).value.length > parseInt(max)) {
		document.getElementById(textarename).value = document
				.getElementById(textarename).value.substring(0, max)
	} else {
		...
		.ready(
				function() {
					if ($('#web-tellafriend-form').length > 0) {
						mesgCounters(300 'edit-message',
								'message-textarea-limit-count');
					...
					if ($('#node-form').length > 0) {
						mesgCounters(300 'edit-field-feedback-body-0-value',
								'feedback-textarea-limit-count');
						$(".messages.error ul li").each(function() {
							if ($(this).find('ul').size() > 0) {
								var cont = $(this).find('ul li').html();
								$(this).find('ul').remove();
					...
					/*
					 * $("#stop").click(function(){ clearTimeout(slideTimeIn);
					 ...
					 * 
					 * $("#play").click(function(){ clearTimeout(slideTimeIn);
					 * _GLOBAL_CNT = 0; $("#fs2").hide();
					 * $("#scroll-content").show(); startscroller(); });
					 * startscroller();
					 */
...
$(document)
		.ready(
				function() {
					...
					$("#views-slideshow-imageflow-images-1_previous").attr(
							"title", "Previous");
					$("#views-slideshow-imageflow-images-1_next").attr("title",
							"Next");
					$(".apachesolr-showhide").attr("title", "Show more");
					if (navigator.userAgent.indexOf('MSIE') < 0
							&& navigator.userAgent.indexOf('Opera') < 0) {
						$("#contentPanel .site-map-box ul li ul li").attr(
								'style', 'padding:0px 40px 0 7px!important;');
					}

					if ($(".block-vrm_customization form")) {
						var frmaction = $(".block-vrm_customization form")
								.attr('action');
						$(".block-vrm_customization form").attr('action',
								frmaction + "?showrating");
					}
...
$(document)
		.ready(
				function() {
					$(".small-catalog-panel .item-list li a.active").parent()
							.css('background-color', '#E3D8BD');
					$(".small-catalog-panel .item-list li a").hover(
							function() {
					...
					if (jQuery("input#edit-keys").val() == "") {
						jQuery(
								".apachesolr_search_message .apachesolr_search_message_text")
								.show();
					} else {
						jQuery(
								".apachesolr_search_message .apachesolr_search_message_text")
...
/*
 * ++++++++++++++++++++++ Dataset Page Tabing Code Starts
 * ++++++++++++++++++++++++
 */
...
jQuery(function() {
	...
	jQuery(".dataset-page .pane-dataset h2 ").each(
			function(i) {
				jQuery(this).css('margin-top',
						datset_tab_default + (datset_tab_inc * i));
			});
