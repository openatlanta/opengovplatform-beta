 ...
 */
function ogpl_india_preprocess_page(&$vars) {
  $vars['tabs2'] = menu_secondary_local_tasks();
  ogpl_india_removetab('Workflow', $vars);
...
function ogpl_india_removetab($label, &$vars) {
  $tabs = explode("\n", $vars['tabs']);
  $vars['tabs'] = '';
...
function phptemplate_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
	...
	if(in_array(drupal_get_path_alias($_GET['q']),array("admin/build/block/list","admin/build/block","admin/build/block/list/ogpl_india"))) {
	unset($breadcrumb['1']);
	unset($breadcrumb['3']);
	unset($breadcrumb['4']);
	$breadcrumb['2']=l('OGPL With CSS2 Theme Blocks','admin/build/block/list/ogpl_india');}
...
function ogpl_india_preprocess_search_theme_form(&$vars,$hook)
{
...
function ogpl_india_search_form($form)
{
  print_r($form);

}
function ogpl_india_menu_item_link($link) {
...
function ogpl_india_apachesolr_facet_link($facet_text, $path, $options = array(), $count, $active = FALSE, $num_found = NULL) {
...
function ogpl_india_apachesolr_unclick_link($facet_text, $path, $options = array()) {
  //apachesolr_js();
...
function ogpl_india_form_element($element, $value) {
...
function ogpl_india_grid_block($element, $name) {
  $output = '';
  if ($element) {
 ...
 */
function ogpl_india_grid_row($element, $name, $class='', $width='', $extra='') {
  $output = '';
  $extra = ($extra) ? ' ' . $extra : '';
  if ($element) {
    if ($class == 'full-width') {
...
function ogpl_india_filefield_icon($file) {
    global $base_url;
    if (is_object($file)) {
        $file = (array) $file;
    }
